...
//        return gob.hasComponents(DrawableComponent.class);
    }
...
//            }
                if (drawable instanceof ParticleFXComponent) {
                    drawParticlePosition(spriteBatch, gob, drawable);
                } else {
                    drawStandardDrawable(spriteBatch, gob, drawable, pos, size);
                }
            }
        }))));
        spriteBatch.end();
    ...
    private void drawParticlePosition(SpriteBatch spriteBatch, MyGameObject gob, DrawableComponent drawable) {
        ParticleFXComponent particleFX = (ParticleFXComponent) drawable;
        particleFX.effect.draw(spriteBatch);
    }

    private void drawStandardDrawable(SpriteBatch spriteBatch, MyGameObject gob, DrawableComponent drawable, PositionComponent pos, SizeComponent size) {
        if (drawable.isVisible) {
            float rotation = gob.getComponent(RotationComponent.class).map(RotationComponent::toDegrees).orElseGet(() -> 0f);
            Vector2 origin = gob.getComponent(OriginComponent.class).map(OriginComponent::toVector2).orElseGet(() -> new Vector2(0, 0)).scl(size.w, size.h);
            spriteBatch.draw(drawable.image(), pos.x - origin.x, pos.y - origin.y, origin.x, origin.y, size.w, size.h, drawable.scaleX, drawable.scaleY, rotation);
